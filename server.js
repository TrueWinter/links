...
try {
  ...
  console.error(`Unable to load config file. Error code: ${e.code}`);
  process.exit(1);
}
...
app.post('/new', function (req, res) {
 //console.log(req.body.pass);
  ...
  if (!req.body.url) {
    ...
    //console.log(regex.test(req.body.url));
    ...
    var check = knex.select('url').from('links').where('id', random).toString();
    //console.log(check);
      db.serialize(function() {
